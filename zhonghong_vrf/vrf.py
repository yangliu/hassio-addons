...
STATES = {'on':{0:'OFF', 1:'ON','OFF':0,'ON':1},
    ...
    'fan':{0:'auto',1:'high',2:'medium',4:'low',5:'auto',6:'silent','auto':0,'high':1,'medium':2,'low':4,'silent':8}}
...
def sync_acs(client):
    ...
    if acs == None:   # setup ac_list if it is empty
        ...
        for ac in acs:
            # Subscribe
            client.subscribe("climate/zhonghong/ac_{0}_{1}/{2}/set".format(ac['oa'], ac['ia'], 'on'))
            client.subscribe("climate/zhonghong/ac_{0}_{1}/{2}/set".format(ac['oa'], ac['ia'], 'mode'))
            client.subscribe("climate/zhonghong/ac_{0}_{1}/{2}/set".format(ac['oa'], ac['ia'], 'temp'))
            client.subscribe("climate/zhonghong/ac_{0}_{1}/{2}/set".format(ac['oa'], ac['ia'], 'fan'))
            # Publish
            if ac['on'] == 1:
                client.publish("climate/zhonghong/ac_{0}_{1}/{2}/state".format(ac['oa'], ac['ia'], 'mode'), STATES['mode'][ac['mode']])
            else:
                client.publish("climate/zhonghong/ac_{0}_{1}/{2}/state".format(ac['oa'], ac['ia'], 'mode'), STATES['mode'][0])
            client.publish("climate/zhonghong/ac_{0}_{1}/{2}/state".format(ac['oa'], ac['ia'], 'temp'), ac['tempSet'])
            ...
            ha_config = {
                ...
                "current_temperature_topic": "climate/zhonghong/ac_{0}_{1}/{2}/state".format(ac['oa'], ac['ia'], 'cur_temp'),
                "power_command_topic": "climate/zhonghong/ac_{0}_{1}/{2}/set".format(ac['oa'], ac['ia'], 'on'),
                "mode_command_topic": "climate/zhonghong/ac_{0}_{1}/{2}/set".format(ac['oa'], ac['ia'], 'mode'),
                "mode_state_topic": "climate/zhonghong/ac_{0}_{1}/{2}/state".format(ac['oa'], ac['ia'], 'mode'),
                "modes": ["off", "cool", "heat", "dry", "fan_only"],
                "temperature_command_topic": "climate/zhonghong/ac_{0}_{1}/{2}/set".format(ac['oa'], ac['ia'], 'temp'),
                "temperature_state_topic": "climate/zhonghong/ac_{0}_{1}/{2}/state".format(ac['oa'], ac['ia'], 'temp'),
                "fan_mode_command_topic": "climate/zhonghong/ac_{0}_{1}/{2}/set".format(ac['oa'], ac['ia'], 'fan'),
                "fan_mode_state_topic": "climate/zhonghong/ac_{0}_{1}/{2}/state".format(ac['oa'], ac['ia'], 'fan'),
                "fan_modes": ["auto", "low", "medium", "high", "silent"],
                "min_temp": 18,
                "max_temp": 30,
                "temp_step": 1,
                "device": {
                    "identifiers": "ZHONGHONG_VRF_Addons",
                    "manufacturer": "Zhonghong",
                    "model": "HVAC_AQARA",
                    "name": "Zhonghong HVAC Hub",
                    "sw_version": "0.1.2"
                }
            }
            client.publish("homeassistant/climate/zhonghong/ac_{0}_{1}/config".format(ac['oa'], ac['ia']),json.dumps(ha_config))
            logging.info ("Publish {0} with {1}".format("homeassistant/climate/zhonghong/ac_{0}_{1}/config".format(ac['oa'], ac['ia']),json.dumps(ha_config)))
            ...
            for ac in acs:
                if ac['oa'] == ac_temp['oa'] and ac['ia'] == ac_temp['ia']:
                    if ac['on'] != ac_temp['on'] or ac['mode'] != ac_temp['mode']:
                        ...
                        topic = "climate/zhonghong/ac_{0}_{1}/{2}/state".format(ac['oa'], ac['ia'], 'mode')
                    ...
                    if ac['tempSet'] != ac_temp['tempSet']:
                        topic = "climate/zhonghong/ac_{0}_{1}/{2}/state".format(ac['oa'], ac['ia'], 'temp')
                        msg = ac_temp['tempSet']
                        client.publish(topic, msg)
                        logging.info (f"Publish `{msg}` to `{topic}` topic")
                    if ac['tempIn'] != ac_temp['tempIn']:
                        topic = "climate/zhonghong/ac_{0}_{1}/{2}/state".format(ac['oa'], ac['ia'], 'cur_temp')
                        msg = ac_temp['tempIn']
                        client.publish(topic, msg)
                        logging.info (f"Publish `{msg}` to `{topic}` topic")
                    if ac['fan'] != ac_temp['fan']:
                        topic = "climate/zhonghong/ac_{0}_{1}/{2}/state".format(ac['oa'], ac['ia'], 'fan')
